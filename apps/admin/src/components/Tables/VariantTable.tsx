...
interface VariantTableProps {
  ...
  onInventoryClick?: (variant: Variant) => void;
}


const VariantTable: React.FC<VariantTableProps> = ({ variants, productId, onEdit, onDelete, onView, onInventoryClick }) => {
  ...
  const paginatedVariants = useMemo(() => variants.slice(startIndex, endIndex), [variants, startIndex, endIndex]);
        ...
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Price Adjustment %</th>
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Images</th>
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Videos</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {paginatedVariants.map((variant) => (
              <tr key={variant.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-6 py-4 text-sm font-medium text-gray-900">{variant.id}</td>
                <td className="px-6 py-4 text-sm text-gray-600">{variant.color}</td>
                <td className="px-6 py-4 text-sm text-gray-600">{variant.sku || '-'}</td>
                <td className="px-6 py-4 text-sm text-gray-600">{variant.addPercentageInBasePrice ? `${variant.addPercentageInBasePrice > 0 ? '+' : ''}${variant.addPercentageInBasePrice}%` : '-'}</td>
                <td className="px-6 py-4 text-sm text-gray-600">{variant.media.images.length}</td>
                <td className="px-6 py-4 text-sm text-gray-600">{variant.media.videos.length}</td>
                ...
                <td className="px-6 py-4 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => onInventoryClick?.(variant)}
                      className="p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors"
                      title="Inventory"
                      disabled={!onInventoryClick}
                    >
                      {renderIcon(FiPackage, { size: 16 })}
                    </button>
                    <button
                      onClick={() => onDelete(variant._id, `${variant.color} - ${variant.sku}`)}
