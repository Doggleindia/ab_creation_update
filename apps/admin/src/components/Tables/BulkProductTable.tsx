...
import { BulkProduct } from '../../types/models';
...
interface BulkProductTableProps {
  ...
  onView: (product: BulkProduct) => void;
}

const BulkProductTable: React.FC<BulkProductTableProps> = ({ products, onEdit, onDelete, onView }) => {
                ...
                <td className="px-6 py-4 text-sm text-gray-600">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {(product.sizes || []).length > 2 && (
                      <span className="text-xs text-gray-500">+{(product.sizes || []).length - 2} more</span>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4 text-sm text-gray-600">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {(product.colors || []).length > 2 && (
                      <span className="text-xs text-gray-500">+{(product.colors || []).length - 2} more</span>
                    )}
                  </div>
                </td>
